      ...
      <div className="card p-6">
        ...
        <form onSubmit={addFact} className="flex gap-2 mt-4">
          <input
            ...
            placeholder='Örn. "2019 yılında İstanbul’da kuruldu"'
            className="input flex-1"
          />
          <button type="submit" className="btn-primary inline-flex items-center gap-1.5 whitespace-nowrap">
        ...
        <div className="mt-4 space-y-2">
          ...
          {facts.map((f) => (
            ...
            >
              ...
              <button onClick={() => delFact(f.id)} className="text-ink-faint hover:text-danger shrink-0">
          ...
          {scan.needsLLM ? (
            ...
            <div className="card p-6 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-[13px] text-ink-muted mt-1">
                  {scan.checked} cevap tarandı; markanız hakkında çelişki bulunamadı.
                </p>
              </div>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="text-[13px] text-ink-muted">
                {scan.checked} cevap tarandı · {scan.hallucinations.length} olası yanlış
              </div>
              {scan.hallucinations.map((h, i) => (
                <div key={i} className="card p-5">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span
                      className={`text-[10.5px] rounded px-1.5 py-0.5 ${h.severity === 'Yüksek' ? 'bg-danger/10 text-danger' : h.severity === 'Orta' ? 'bg-warning/10 text-warning' : 'bg-paper-3 text-ink-muted'}`}
                    >
                      {h.severity}
                    </span>
                  ...
                  <div className="text-[13.5px] text-ink">
                    <span className="text-danger">Yanlış:</span> {h.claim}
                  </div>
                  {h.correction && (
                    <div className="text-[13px] text-positive mt-1">
                      <span className="font-medium">Doğrusu:</span> {h.correction}
                    </div>
                  )}
                </div>
              ))}
...
function Notice({ tone, children }: { tone: 'warn'; children: React.ReactNode }) {
  void tone;
  return (
    <div className="text-[13px] text-warning bg-warning/5 border border-warning/20 rounded-lg p-4">{children}</div>
  );
}
