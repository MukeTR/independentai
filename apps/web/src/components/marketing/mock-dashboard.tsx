...
export function MockDashboard() {
  ...
  const w = 480;
  const h = 100;
  const points = trend.map((v, i) => `${(i * w) / (trend.length - 1)},${h - (v / max) * h * 0.8}`).join(' ');

  return (
    <div
      className="card bg-paper-3 p-6 shadow-[0_30px_80px_-20px_rgba(20,17,13,0.15)] max-w-full overflow-hidden"
      aria-label="Örnek panel görünümü (demo veri)"
    >
      {/* topbar */}
      <div className="flex items-center justify-between border-b-hairline border-hairline pb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="w-2 h-2 rounded-full bg-paper-4" />
          <span className="w-2 h-2 rounded-full bg-paper-4" />
          <span className="text-[11px] text-ink-faint font-mono ml-3 truncate">independentai.space/dashboard</span>
        </div>
      ...
      <div className="grid grid-cols-12 gap-4 mt-5">
        ...
        <div className="col-span-4 border-hairline border rounded-lg p-3">
          ...
          <div className="font-display text-[28px] tabular text-brand mt-1">
            68<span className="text-[14px] text-ink-faint">%</span>
          </div>
        </div>
        <div className="col-span-4 border-hairline border rounded-lg p-3">
          <div className="text-[9px] text-ink-faint tracking-wider uppercase">Share of Voice</div>
          <div className="font-display text-[28px] tabular mt-1">
            41<span className="text-[14px] text-ink-faint">%</span>
          </div>
        </div>
        <div className="col-span-4 border-hairline border rounded-lg p-3">
        ...
        <div className="col-span-8 border-hairline border rounded-lg p-4">
          ...
          <svg
            ...
            role="img"
            aria-label="14 günlük görünürlük trendi, yükseliş eğiliminde"
          >
            <defs>
            ...
            {trend.map((v, i) => (
              <circle key={i} cx={(i * w) / (trend.length - 1)} cy={h - (v / max) * h * 0.8} r="2" fill="#4F46E5" />
            ))}
          </svg>
        </div>
        ...
        <div className="col-span-12 border-hairline border rounded-lg p-4">
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div
                key={i}
                className="flex items-center justify-between border-b-hairline border-hairline last:border-0 pb-2 last:pb-0"
              >
                <span className="text-[12px] truncate min-w-0">{p.q}</span>
